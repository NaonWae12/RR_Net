...
const navItems: NavItem[] = [
  // Finance-specific menu items (placed at top for priority)
  {
  ...
  },
  {
    label: 'My Workspace',
    href: '/dashboard',
    // Employee self-service dashboard - available for all roles
    // HR and Finance also need this for personal attendance, reimbursement, time-off, and payslip
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...

  {
    ...
    restrictedRoles: ['owner', 'admin', 'hr', 'finance'],
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    label: 'Settlement',
    href: '/settlement',
    // Admin bisa akses, tapi finance sekarang dilarang (request user)
    allowedRoles: ['owner', 'admin'],
    restrictedRoles: ['technician', 'hr', 'finance'], 
    requiredAnyFeatures: ['settlement'],
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  },
  {
    ...
    href: '/reseller',
    allowedRoles: ['owner', 'admin'],
    restrictedRoles: ['hr', 'finance'],
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 110-6 3 3 0 010 6z" />
      </svg>
    ),
  },
  {
    ...
    requiredAnyFeatures: ['mikrotik_api_basic', 'mikrotik_api'],
    allowedRoles: ['owner', 'admin'],
    restrictedRoles: ['hr', 'finance'], // HR and Finance don't need PPPoE access
    icon: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export function Sidebar({ isOpen, onClose }: SidebarProps) {
  ...
  const { role, originalRole, switched, roleContext, isAdmin } = useRole();
        ...
        <nav className="p-4 space-y-1">
          {visibleNavItems.map((item) => {
            ...
            if (item.href === '/dashboard') {
              if (role === 'technician') label = 'Technician Dashboard';
              else if (role === 'hr') label = 'Employee Desk';
              else if (role === 'finance') label = 'Personal Desk';
              else if (isAdmin) label = 'Admin Dashboard';
            }
            
            if (item.href === '/technician/tasks' && role === 'technician') {
              label = 'My Tasks';
            }
