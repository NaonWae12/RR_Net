...
const ModernTemplate: React.FC<VoucherTemplateProps> = ({ voucher, index = 0, pkg, headerTitle, config }) => {
  ...
  return (
    <div className="print-card bg-white border border-dashed border-slate-600 p-0 text-slate-900 w-full max-w-[160px] shadow-none flex flex-col overflow-hidden leading-none font-sans relative h-[88px]">
      ...
      <div className="flex items-center gap-1 p-0.5 border-b-2 border-slate-200 bg-slate-50/50 h-[22px]">
      ...
      <div className="flex flex-1 relative overflow-hidden">
        {/* Left Side: Date & QR */}
        <div className="w-[40%] border-r-2 border-slate-200 flex flex-col items-center justify-start bg-slate-50/30 h-full pt-1">
        ...
        <div className="w-6 border-l-2 border-slate-200 flex flex-col items-center justify-start py-1 bg-slate-50/80 h-full">
           ...
           {/* Index Voucher */}
           <div className="mt-auto w-4 h-4 rounded-full border border-black flex items-center justify-center bg-black text-white font-black text-[7px] shrink-0 mb-2">
             {index + 1}
           </div>
        </div>
      </div>
